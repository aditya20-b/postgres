...
// Keep only necessary includes for the reverted GetTableAmRoutine and GUC check
#include "access/tableam.h"
#include "access/xact.h"        // For IsTransactionState
#include "commands/defrem.h"    // For get_table_am_oid
#include "miscadmin.h"          // For MyDatabaseId
#include "utils/builtins.h"     // For DatumGetPointer, OidFunctionCall0
#include "utils/guc_hooks.h"
#include "utils/lsyscache.h"    // For get_am_oid (used in GUC check)
 ...
 *		TableAmRoutine struct. The input is the OID of the AM handler
 *		function itself (from pg_proc.oid, stored in pg_am.amhandler).
 */
const TableAmRoutine *
GetTableAmRoutine(Oid amhandler_oid)
{
	Datum		datum;
	 ...
	 */
	datum = OidFunctionCall0(amhandler_oid);
